...
(defn result-stopped? [{:keys [execution effects error]}]
  (or (boolean error)
...
(defn step-execution
  ...
  Types:
    Returns {:execution next-execution, :effects [{:op :kw, :args ..., :complete-ref resume-id}], :transitions [...], :error ...}
      ...
      transitions are the sequence of [{:id action-id}] that indicates what
        path was taken produces the associated execution.

        Also, each element will have metadata that has the following keys:
          :execution - the resulting execution after this action
          :effects - the pending effects generated by this action
          :error - the error generated by this action
